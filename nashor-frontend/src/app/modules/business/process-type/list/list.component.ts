...
export class ProcessTypeListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._processTypeService
      ...
      .subscribe((processTypes: ProcessType[]) => {
         ...
         * Update the counts
         */
        this.count = processTypes.length;
        /**
         * Mark for check
         */
   ...
   */
  goToEntity(id_process_type: string): void {
    /**
    ...
    this._router.navigate(
      [this.openMatDrawer ? '../' : './', id_process_type],
      { relativeTo: route }
    );
    /**
     * Mark for check
   ...
   * createProcessType
   */
  createProcessType(): void {
    this._angelConfirmationService
      .open({
        title: 'Añadir tipo de proceso',
        message:
          '¿Estás seguro de que deseas añadir una nueva tipo de proceso? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
            ...
            .subscribe({
              next: (_processType: ProcessType) => {
                if (_processType) {
                  ...
                  /**
                   * Go to new process_type
                   */
                  this.goToEntity(_processType.id_process_type);
                } else {
