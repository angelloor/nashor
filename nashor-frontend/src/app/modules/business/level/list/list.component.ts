...
import { ModalSelectTemplateService } from '../../template/modal-select-template/modal-select-template.service';
...
export class LevelListComponent implements OnInit {
   ...
   * @param id_level
   */
  goToEntity(id_level: string): void {
    /**
     ...
     * Go to level
     */
    this._router.navigate([this.openMatDrawer ? '../' : './', id_level], {
      relativeTo: route,
    });
    /**
     * Mark for check
   ...
   */
  createLevel(): void {
    this._modalSelectTemplateService
      .openModalSelectTemplate()
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((id_template: string) => {
        if (id_template) {
           ...
           */
          this._levelService
            .create(id_user_, id_company, id_template)
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe({
              next: (_level: Level) => {
                console.log(_level);
                if (_level) {
                   ...
                   * Go to new level
                   */
                  this.goToEntity(_level.id_level);
                } else {
