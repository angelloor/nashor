...
export class PositionListComponent implements OnInit {
   ...
   * @param id_position
   */
  goToEntity(id_position: string): void {
    /**
     ...
     * Go to position
     */
    this._router.navigate([this.openMatDrawer ? '../' : './', id_position], {
      relativeTo: route,
    });
    /**
     * Mark for check
  ...
  createPosition(): void {
    this._angelConfirmationService
      .open({
        title: 'Añadir cargo',
        message:
          '¿Estás seguro de que deseas añadir una nueva cargo? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
            ...
            .subscribe({
              next: (_position: Position) => {
                if (_position) {
                   ...
                   * Go to new position
                   */
                  this.goToEntity(_position.id_position);
                } else {
