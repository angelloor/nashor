...
export class OfficialListComponent implements OnInit {
   ...
   * @param id_official
   */
  goToEntity(id_official: string): void {
    /**
     ...
     * Go to official
     */
    this._router.navigate([this.openMatDrawer ? '../' : './', id_official], {
      relativeTo: route,
    });
    /**
     * Mark for check
  ...
  createOfficial(): void {
    this._angelConfirmationService
      .open({
        ...
        message:
          '¿Estás seguro de que deseas añadir una nueva funcionario? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
            ...
            .subscribe({
              next: (_official: Official) => {
                ...
                if (_official) {
                   ...
                   * Go to new official
                   */
                  this.goToEntity(_official.id_official);
                } else {
