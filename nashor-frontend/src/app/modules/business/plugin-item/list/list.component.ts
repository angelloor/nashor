...
@Component({
  selector: 'plugin-item-list',
  templateUrl: './list.component.html',
})
export class PluginItemListComponent implements OnInit {
  @ViewChild('matDrawer', { static: true }) matDrawer!: MatDrawer;
  count: number = 0;
  pluginItems$!: Observable<PluginItem[]>;
  ...
  ngOnInit(): void {
     ...
     * Get the pluginItems
     */
    this.pluginItems$ = this._pluginItemService.pluginItems$;
    /**
     *  queryRead *
     */
    ...
    this.searchInputControl.valueChanges
      .pipe(
        ...
        switchMap((query) => {
           ...
           * Search
           */
          return this._pluginItemService.byCompanyQueryRead(
            this.id_company,
            query.toLowerCase()
          );
        })
   ...
   */
  goToEntity(id_plugin_item: string): void {
    /**
     ...
     * Go to plugin_item
     */
    this._router.navigate([this.openMatDrawer ? '../' : './', id_plugin_item], {
      relativeTo: route,
    });
    /**
     * Mark for check
   ...
   * createPluginItem
   */
  createPluginItem(): void {
    this._angelConfirmationService
      .open({
        title: 'Añadir item plugin',
        message:
          '¿Estás seguro de que deseas añadir una nueva item plugin? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
            ...
            .subscribe({
              next: (_pluginItem: PluginItem) => {
                ...
                if (_pluginItem) {
                  ...
                  /**
                   * Go to new plugin_item
                   */
                  this.goToEntity(_pluginItem.id_plugin_item);
                } else {
