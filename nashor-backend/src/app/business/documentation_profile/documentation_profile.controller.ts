...
export const validation = (
	...
	return new Promise<
		DocumentationProfile | DocumentationProfile[] | boolean | any
			...
			await verifyToken(token)
				.then(async () => {
					...
					if (url == '/create' || url == '/update') {
						attributeValidate(
							'id_user_',
							documentation_profile.id_user_,
							'number',
							10
						).catch((err) => {
					...
					if (!validationStatus) {
						 ...
						 * Instance the class
						 */
						const _documentation_profile = new DocumentationProfile();
						/**
						 * Execute the url depending on the path
						 */
						if (url == '/create') {
							/** set required attributes for action */
							_documentation_profile.id_user_ = documentation_profile.id_user_;
							_documentation_profile.company = documentation_profile.company;
							await _documentation_profile
								.create()
								.then((_documentationProfile: DocumentationProfile) => {
									resolve(_documentationProfile);
								})
								.catch((error: any) => {
									reject(error);
								});
							...
							/** set required attributes for action */
							_documentation_profile.name_documentation_profile =
								documentation_profile.name_documentation_profile;
							await _documentation_profile
								.queryRead()
								.then((_documentationProfiles: DocumentationProfile[]) => {
							...
							const id_company: any = documentation_profile.company;
							...
							if (id_company >= 1) {
								...
								await _documentation_profile
									.byCompanyQueryRead()
									.then((_documentation_profiles: DocumentationProfile[]) => {
										resolve(_documentation_profiles);
									})
									.catch((error: any) => {
										reject(error);
									});
							...
							if (id_documentation_profile >= 1) {
								/** set required attributes for action */
								_documentation_profile.id_documentation_profile =
									documentation_profile.id_documentation_profile;
								await _documentation_profile
									.specificRead()
									.then((_documentationProfile: DocumentationProfile) => {
							...
							_documentation_profile.name_documentation_profile =
								documentation_profile.name_documentation_profile;
							_documentation_profile.description_documentation_profile =
								documentation_profile.description_documentation_profile;
							_documentation_profile.status_documentation_profile =
								documentation_profile.status_documentation_profile;
							await _documentation_profile
								.update()
								.then((_documentationProfile: DocumentationProfile) => {
