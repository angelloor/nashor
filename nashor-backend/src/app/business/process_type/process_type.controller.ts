import { verifyToken } from '../../../utils/jwt';
import { _messages } from '../../../utils/message/message';
import { ProcessType } from './process_type.class';

export const validation = (
	process_type: ProcessType,
	url: string,
	token: string
) => {
	return new Promise<ProcessType | ProcessType[] | boolean | any>(
		async (resolve, reject) => {
			/**
			 * Capa de Autentificación con el token
			 */
			let validationStatus: boolean = false;

			if (token) {
				await verifyToken(token)
					.then(async () => {
						/**
						 * Capa de validaciones
						 */
						if (url == '/create' || url == '/update') {
							attributeValidate(
								'id_user_',
								process_type.id_user_,
								'number',
								10
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						if (url == '/update') {
							attributeValidate(
								'id_process_type',
								process_type.id_process_type,
								'number',
								5
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						if (url == '/update') {
							attributeValidate(
								'name_process_type',
								process_type.name_process_type,
								'string',
								100
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						if (url == '/update') {
							attributeValidate(
								'description_process_type',
								process_type.description_process_type,
								'string',
								250
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						if (url == '/update') {
							attributeValidate(
								'acronym_process_type',
								process_type.acronym_process_type,
								'string',
								20
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						if (url == '/update') {
							attributeValidate(
								'sequential_process_type',
								process_type.sequential_process_type,
								'number',
								10
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						/**
						 * Validation company
						 */

						if (url == '/create' || url == '/update') {
							attributeValidate(
								'id_company',
								process_type.company.id_company,
								'number',
								5
							).catch((err) => {
								validationStatus = true;
								reject(err);
							});
						}

						/**
						 * Continuar solo si no ocurrio errores en la validación
						 */
						if (!validationStatus) {
							/**
							 * Instance the class
							 */
							const _process_type = new ProcessType();
							/**
							 * Execute the url depending on the path
							 */
							if (url == '/create') {
								/** set required attributes for action */
								_process_type.id_user_ = process_type.id_user_;
								_process_type.company = process_type.company;
								await _process_type
									.create()
									.then((_processType: ProcessType) => {
										resolve(_processType);
									})
									.catch((error: any) => {
										reject(error);
									});
							} else if (url.substring(0, 10) == '/queryRead') {
								/** set required attributes for action */
								_process_type.name_process_type =
									process_type.name_process_type;
								await _process_type
									.queryRead()
									.then((_processTypes: ProcessType[]) => {
										resolve(_processTypes);
									})
									.catch((error: any) => {
										reject(error);
									});
							} else if (url.substring(0, 19) == '/byCompanyQueryRead') {
								const id_company: any = process_type.company;

								if (id_company >= 1) {
									/** set required attributes for action */
									_process_type.company = process_type.company;
									_process_type.name_process_type =
										process_type.name_process_type;
									await _process_type
										.byCompanyQueryRead()
										.then((_process_types: ProcessType[]) => {
											resolve(_process_types);
										})
										.catch((error: any) => {
											reject(error);
										});
								} else {
									reject({
										..._messages[11],
										description: _messages[11].description.replace(
											'name_entity',
											'company'
										),
									});
								}
							} else if (url.substring(0, 13) == '/specificRead') {
								const id_process_type: any = process_type.id_process_type;

								if (id_process_type >= 1) {
									/** set required attributes for action */
									_process_type.id_process_type = process_type.id_process_type;
									await _process_type
										.specificRead()
										.then((_processType: ProcessType) => {
											resolve(_processType);
										})
										.catch((error: any) => {
											reject(error);
										});
								} else {
									reject({
										..._messages[11],
										description: _messages[11].description.replace(
											'name_entity',
											'process_type'
										),
									});
								}
							} else if (url == '/update') {
								/** set required attributes for action */
								_process_type.id_user_ = process_type.id_user_;
								_process_type.id_process_type = process_type.id_process_type;
								_process_type.company = process_type.company;
								_process_type.name_process_type =
									process_type.name_process_type;
								_process_type.description_process_type =
									process_type.description_process_type;
								_process_type.acronym_process_type =
									process_type.acronym_process_type;
								_process_type.sequential_process_type =
									process_type.sequential_process_type;
								await _process_type
									.update()
									.then((_processType: ProcessType) => {
										resolve(_processType);
									})
									.catch((error: any) => {
										reject(error);
									});
							} else if (url.substring(0, 7) == '/delete') {
								/** set required attributes for action */
								_process_type.id_user_ = process_type.id_user_;
								_process_type.id_process_type = process_type.id_process_type;
								await _process_type
									.delete()
									.then((response: boolean) => {
										resolve(response);
									})
									.catch((error: any) => {
										reject(error);
									});
							}
						}
					})
					.catch((error) => {
						reject(error);
					});
			} else {
				reject(_messages[5]);
			}
		}
	);
};

/**
 * Función para validar un campo de acuerdo a los criterios ingresados
 * @param attribute nombre del atributo a validar
 * @param value valor a validar
 * @param type tipo de dato correcto del atributo (string, number, boolean, object)
 * @param _length longitud correcta del atributo
 * @returns true || error
 */
const attributeValidate = (
	attribute: string,
	value: any,
	type: string,
	_length: number = 0
) => {
	return new Promise<Boolean>((resolve, reject) => {
		if (value != undefined || value != null) {
			if (typeof value == `${type}`) {
				if (typeof value == 'string' || typeof value == 'number') {
					if (value.toString().length > _length) {
						reject({
							..._messages[8],
							description: _messages[8].description
								.replace('_nameAttribute', `${attribute}`)
								.replace('_expectedCharacters', `${_length}`),
						});
					} else {
						resolve(true);
					}
				} else {
					resolve(true);
				}
			} else {
				reject({
					..._messages[7],
					description: _messages[7].description
						.replace('_nameAttribute', `${attribute}`)
						.replace('_expectedType', `${type}`),
				});
			}
		} else {
			reject({
				..._messages[6],
				description: _messages[6].description.replace(
					'_nameAttribute',
					`${attribute}`
				),
			});
		}
	});
};
